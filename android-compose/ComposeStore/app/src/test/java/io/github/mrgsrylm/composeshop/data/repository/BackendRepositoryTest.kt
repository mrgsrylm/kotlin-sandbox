...
class BackendRepositoryTest {
    ...
    @Mock
    private lateinit var mockApi: BackendService

    @Mock
    private lateinit var mockDao: ShopDao

    @Before
    fun setup() {
        repository = BackendRepositoryImpl(mockApi, mockDao)
    }
    ...
    fun `test login function`() = runBlocking {
        ...
        `when`(mockApi.login(LoginRequest(username, password))).thenReturn(
            Response.success(
                userDto
            )
        )
